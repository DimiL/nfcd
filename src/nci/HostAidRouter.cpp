...
//singleton HostAidRouter object
HostAidRouter HostAidRouter::sHostAidRouter;
...
bool HostAidRouter::addPpseRoute()
{
...

  {
    ...
    if (nfaStat == NFA_STATUS_OK) {
      //wait for NFA_CE_REGISTERED_EVT
      mRegisterEvent.wait();
...
bool HostAidRouter::deleteAllRoutes()
{
  ...
  for (AidHandleDatabase::iterator iter1 = mHandleDatabase.begin();
    ...
    if (nfaStat == NFA_STATUS_OK) {
      //wait for NFA_CE_DEREGISTERED_EVT
      mDeregisterEvent.wait();
    } else {
